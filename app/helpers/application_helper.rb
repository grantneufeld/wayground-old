...
module ApplicationHelper
	...
	# TODO: Future: There’s probably some far more clever ruby/rails way to do these date/time formatting functions
	# return a formatted datetime string
	def format_datetime(t, format=:default, showtz=false)
		if format.is_a? String
			t.strftime format
		elsif format.is_a? Symbol
			case format
			when :default
				t.strftime "%a, %b %e, %Y, %l:%M %p#{showtz ? ' %Z' : ''}"
			when :long
				t.strftime "%A, %B %e, %Y, %l:%M %p#{showtz ? ' %Z' : ''}"
			when :simple
				t.strftime "%B %e, %Y, %l:%M %p#{showtz ? ' %Z' : ''}" # long without weekday
			when :short
				t.strftime "%Y/%m/%d %H:%M#{showtz ? ' %Z' : ''}"
			when :tight
				t.strftime "%Y/%b/%d %l:%M%p#{showtz ? ' %Z' : ''}"
			else
				t
			end
		else
	...
	def format_date(t, format=:default, showtz=false)
		if format.is_a? String
			t.strftime format
		elsif format.is_a? Symbol
			case format
	...
	def format_time(t, format=:default, showtz=false)
		if format.is_a? String
			t.strftime format
		elsif format.is_a? Symbol
			if format == :simple
			...
			case format
			when :default
				t.strftime "%l:%M %p#{showtz ? ' %Z' : ''}"
			when :long
				# "%I:%M %p"
				t.strftime "%l:%M %p#{showtz ? ' %Z' : ''}"
			when :short
				t.strftime "%H:%M#{showtz ? ' %Z' : ''}"
			when :tight
				t.strftime "%l:%M%p#{showtz ? ' %Z' : ''}"
			else
				t
